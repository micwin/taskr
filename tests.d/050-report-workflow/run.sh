...
grep -q "^## Tasks$" "${stdout}"
grep -q "^Oldest developing: 003 \\[developing\\] #(release,website) Define workflows$" "${stdout}"
grep -q "^Newest done: 002 \\[done\\] #internal Define directory structure$" "${stdout}"
grep -q "^## Subtasks$" "${stdout}"
grep -q "^Oldest designing: 004 \\[designing\\] #copy Define selectors$" "${stdout}"
grep -q "^# Milestones$" "${stdout}"
